...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: product.description?.slice(0, 160) || `Buy ${product.name} at Cold Flyer. Best price in Bangladesh. ${product.category || ''} ${product.brand || ''}`.slice(0, 160),
    openGraph: {
      title: `${product.name} | Cold Flyer AC Shop`,
...
export default async function ProductPage({ params }) {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: product.category || "Products", url: `https://coldflyer.com/items?category=${encodeURIComponent(product.category || '')}` },
