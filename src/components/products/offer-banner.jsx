    ...
    <div className="relative overflow-hidden rounded-xl bg-linear-to-r from-primary to-inverted -mt-8">
      ...
      <div className="relative z-10 px-5 py-4 sm:px-7 sm:py-5 md:px-10 md:py-6">
        <div className="flex flex-col items-center gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div>
              ...
              <div>
                <div className="flex items-baseline gap-2 justify-center md:justify-start flex-wrap">
                  <span className="text-xl font-extrabold tracking-tight text-primary-foreground md:text-2xl">
                    {coupon.description || t("summerSale")}
                  </span>
                  <span className="text-lg font-black text-primary-foreground/90 md:text-xl">{discountLabel}</span>
                  <span className="hidden md:inline-flex items-center rounded border-2 border-green-400 bg-green-500 px-1.5 py-px text-[9px] font-bold text-white rotate-2 leading-none">
                    {t("exclusive")}
                  </span>
                </div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <div className="flex items-center gap-1.5 rounded-lg border border-primary-foreground/30 bg-primary-foreground/10 px-3 py-1.5">
              <span className="text-xs font-bold text-primary-foreground/80">{t("useCode")}:</span>
              <span className="text-sm font-extrabold tracking-wider text-primary-foreground">{coupon.code}</span>
              <button
                onClick={() => handleCopyCode(coupon.code)}
                className="ml-1 rounded bg-primary-foreground/20 px-1.5 py-0.5 text-[10px] font-bold text-primary-foreground hover:bg-primary-foreground/30 transition-colors"
              >
                {t("copyCode")}
              </button>
            </div>
