...
export default function CartExample_01() {
  ...
  const [shippingCost] = useState(15.99);
  const [vatRate] = useState(0.2); // 20% VAT rate
  const [isLoading, setIsLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState("");

  // Calculate subtotal whenever the products change
  useEffect(() => {
    const newSubtotal = products.reduce((total, product) => total + product.price * product.quantity, 0);
  ...
  const handleContinueShopping = (payload) => {
